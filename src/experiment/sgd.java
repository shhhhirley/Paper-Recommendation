...
public class sgd {
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException {
		...
		File la = new File("/Users/apple/desktop/paper_recommendation/results_1000/keywords_labels.txt");
		File f = new File("/Users/apple/desktop/paper_recommendation/data/author_paper_id.txt");
		File k = new File("/Users/apple/desktop/paper_recommendation/results_1000/keywords_vectors.txt");
		File v = new File("/Users/apple/desktop/paper_recommendation/results_1000/venues_vectors.txt");
		File a = new File("/Users/apple/desktop/paper_recommendation/results_1000/authors_vectors.txt");
		...
		File la2 = new File("/Users/apple/desktop/paper_recommendation/results_1000/venue_labels.txt");
		File la3 = new File("/Users/apple/desktop/paper_recommendation/results_1000/author_labels.txt");
...
// ------------------------------------------------------------------------get_keywords
		...
		String paper_list[] = new String[paper_key.size()]; 
		paper_key.keySet().toArray(paper_list);
		bl.close();
...
//------------------------------------------------------------------------------迭代		
		...
		{	
			...
			double neg_obj[] = new double[5];
			for(int sc = 0; sc < 5000; sc++)
			{
			ArrayList<Integer> re_key = paper_key.get(refs[w]);
			ArrayList<Integer> pa_key = paper_key.get(paps[w]);
...
//			
			...
			if (venues.containsKey(venue.get(refs[w])))
			{
				...
				{
					int temp = (int) it.next();
					if(pa_key.contains(temp))
						keywords.put(temp, keywords.get(temp).add(delta_xk.mapMultiply(step)));
					else
						keywords.put(temp, keywords.get(temp).add(delta_xk1.mapMultiply(step)));
				}
				for (Iterator it = pa_key.iterator(); it.hasNext();)
				{
					int temp = (int) it.next();
					if(re_key.contains(temp))
						keywords.put(temp, keywords.get(temp).add(delta_xk.mapMultiply(step)));
					else
						keywords.put(temp, keywords.get(temp).add(delta_xk2.mapMultiply(step)));
				}
	...
	//			authors.put(auths[w], pa_auth);
				...
				c += 2*prefix*step;
				obj += lg.value(s.value(fpa+grp))/5000;
...
//-------------------------------------------------------------------------------负采样的迭代
				...
				for(int j = 0; j < m; j++)
				{
					...
					neg_pap = paper_list[rd.nextInt(paper_list.length)];
					...
					{
					//	System.out.println("author repeated");
						neg_pa_auth = authors.get(neg_auth);
					}
						
					else {
						//System.out.println("author not repeated");
						for (int i = 0; i < 50; i++)
					...
					for (Iterator it = re_key.iterator(); it.hasNext();) 
							xkr = xkr.add(keywords.get(it.next()));
					xkr = (ArrayRealVector) xkr.mapDivide(re_key.size());
					for (Iterator it = neg_pa_key.iterator(); it.hasNext();) {
						int temp = (int) it.next();
						if (keywords.containsKey(temp))
							neg_xkp = neg_xkp.add(keywords.get(temp));
						else {
							//System.out.println("keywords not repeated");
							ArrayRealVector neg_pa_keywords = new ArrayRealVector(50);
							for (int u = 0; u < 50; u++)
								neg_pa_keywords.addToEntry(u, 2 * rd.nextDouble() - 1);
							keywords.put(temp, neg_pa_keywords);
					...
					ArrayRealVector neg_postfix = (ArrayRealVector) neg_pa_auth.mapMultiply(alpha / neg_pa_key.size())
							.add(neg_xp.mapMultiply(alpha / re_key.size())).add(xr.mapMultiply(alpha / neg_pa_key.size()))
							.add(p.mapMultiply(alpha / neg_pa_key.size())).add(q.mapMultiply(alpha / re_key.size()));
					ArrayRealVector neg_postfix1 = (ArrayRealVector) neg_xp.mapMultiply(alpha / re_key.size()).add(q.mapMultiply(alpha / re_key.size()));
					ArrayRealVector neg_postfix2 = (ArrayRealVector) neg_pa_auth.mapMultiply(alpha / neg_pa_key.size())
							.add(xr.mapMultiply(alpha / neg_pa_key.size())).add(p.mapMultiply(alpha / neg_pa_key.size()));
					...
					{
						int temp = (int) it.next();
						if(neg_pa_key.contains(temp))
							keywords.put(temp, keywords.get(temp).add(neg_delta_xk.mapMultiply(step)));
						else
							keywords.put(temp, keywords.get(temp).add(neg_delta_xk1.mapMultiply(step)));
					}
					for (Iterator it = neg_pa_key.iterator(); it.hasNext();)
					{
						int temp = (int) it.next();
						if(re_key.contains(temp))
							...
							keywords.put(temp, keywords.get(temp).add(neg_delta_xk2.mapMultiply(step)));
					}
						...
						c += 2*neg_prefix*step;
						neg_obj[j] += lg.value(1-s.value(neg_fpa+neg_grp))/5000;
				}
